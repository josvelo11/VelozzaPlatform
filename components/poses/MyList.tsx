...
import { generatePocketGuidePdf } from '@/lib/pdf/pocket-guide';

interface Item { n: number; slug: string; guide: string; star: boolean; h1: string; img: string; tell: string; tip: string; altText: string }
export function MyList({ items, base }: { items: Item[]; base: string }) {
  ...
  async function download() {
    setPdfState('working'); setPct(0);
    try {
      await generatePocketGuidePdf(
        mine.map((i) => ({ n: i.n, h1: i.h1, img: i.img, tip: i.tip, tell: i.tell, altText: i.altText })),
        setPct
      );
      setPdfState('idle');
    } catch (e) {
  ...
  return (
    <>
      ...
      <p className="section-lead">Estas son las poses que guardaste. Descarga tu propio PDF con el diseño de Velozza, envíalo a tu fotógrafo o llévalo el día de tu boda. Son ilustraciones creadas con IA, solo como referencia.</p>
      {mine.length === 0 ? <p>Aún no has guardado poses. <Link href={base}>Explora la guía</Link> y toca «Guardar en mi lista».</p> : (
        <>
          <div className="pz-actions pz-noprint">
            <button className="cta-primary" onClick={download} disabled={pdfState === 'working'}>
              {pdfState === 'working' ? `Generando tu PDF… ${pct}%` : `Descargar mi PDF (${mine.length} poses)`}
            </button>
            <a className="cta-secondary" href={`https://api.whatsapp.com/send?text=${encodeURIComponent(text)}`} target="_blank" rel="noreferrer">Enviar por WhatsApp</a>
          ...
          {pdfState === 'err' && <p className="pz-err">No se pudo generar el PDF. Intenta de nuevo o escríbenos por WhatsApp.</p>}
