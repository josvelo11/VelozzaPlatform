#!/usr/bin/env node
// gen-sesion-pareja-l3.mjs — Lección 3: "Posar como equipo" (3 imágenes)
...
const DIPTYCH = (left, right, shot = 'plano de cuerpo entero, ambos en el encuadre') => `Genera una imagen. Díptico editorial dividido en dos mitades verticales EXACTAMENTE iguales en tamaño, formato horizontal 4:3, ambos paneles ${shot}, MISMA distancia de cámara y altura de encuadre, misma pareja en ambos. IZQUIERDA: ${W} y ${M}, ${left}. DERECHA: la misma pareja, mismo encuadre, ${right}. ${STYLE}`;

const jobs = [
  {
    id: 'l3-0',
    prompt: SINGLE('de pie perfectamente rectos y paralelos como dos columnas separadas, espaciados uniformemente, sin ningún punto de contacto ni interacción entre ellos', 'plano de cuerpo entero, ambos de pie separados'),
  },
  {
    id: 'l3-1',
    prompt: DIPTYCH(
      'de pie perfectamente rectos y simétricos, formando dos líneas paralelas rígidas, sin curvas ni inclinación',
      'abrazados con los cuerpos relajados formando una diagonal suave, uno inclinado levemente hacia el otro, asimétricos y naturales',
    ),
  },
  {
    id: 'l3-2',
    prompt: SINGLE('ella de pie sobre un pequeño escalón de piedra natural para alinear sus cabezas en diagonal con él, ambos con el peso del cuerpo desplazado hacia una pierna, postura relajada y en equipo', 'plano de cuerpo entero usando el desnivel del escalón'),
  },
];
...
async function main() {
  console.log(`Generando ${jobs.length} imágenes de la Lección 3 (posar como equipo) — sesion-pareja\n`);
  let ok = 0;
